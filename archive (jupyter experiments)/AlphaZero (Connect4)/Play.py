...
class Play:
    ...
    def draw_move(self, move=None):
        ...
        # Assuming self.game and other necessary attributes are properly defined.
        i, j = self.game.last_move if move is None else move
        c = 'salmon' if self.player == 1 else 'lightskyblue'

        # Scatter plot for the move.
        self.ax.scatter(i, j, s=500, marker='o', zorder=3, c=c)

        # Draw winner or additional game-related updates.
        score = self.game.score
        self.draw_winner(score)

        # Use canvas.draw() to update the figure interactively.
    ...
    def draw_winner(self, score):
        ...
        if score == -1 or score == 1:
            locs = self.game.get_winning_loc()  # Ensure this returns coordinates.
            c = 'darkred' if score == 1 else 'darkblue'

            # Scatter plot for the winning locations.
            for loc in locs:
                self.ax.scatter(loc[0], loc[1], s=300, marker='*', c=c, zorder=4)

        # Disconnect interactive events if the game is over, optional.
